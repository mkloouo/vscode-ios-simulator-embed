...
- **Batched typing** (compose buffer) now maps **full US QWERTY ASCII** punctuation and shifted number-row symbols, not only letters, digits, and space — fixes characters that were dropped when coalesced into `simKeyBatch`.
- **Single key events** with missing or unknown `event.code` fall back to the same character-based mapping; **Shift** / **Alt** inside that mapping is not doubled when the browser also reports those modifiers.
- **`IntlBackslash`** (ISO `\` / `|`) added to `ev.code` → HID map.
- **£** (U+00A3) and **§** (U+00A7) use approximate **US Mac Option** sequences (Option+3 / Option+6); other layouts may differ.
